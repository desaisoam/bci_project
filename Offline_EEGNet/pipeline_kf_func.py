...
from EEGData import EEGData, CachedEEGData
...
def pipeline(config, model_name=None, data_name=None, verbosity=1, delete_data=False, train_kf=True):
    ...
    # ==================== Preparation ====================
    # Read in config file
    # Deprecated. config is passed as dict.
    ...
    h5_path = config['h5_dir'] + data_name + '.h5'
    print('h5_path', h5_path, 'model_name', model_name)
    ...
    utils.dump_to_yaml(config, model_dir + 'config.yaml')
    ...
    try:
        ...
        for validation_fold in range(config_train['num_folds']):
            ...
            if config_train['train_num_workers'] == 0:
                train_dataset = CachedEEGData(h5_path, train_folds)
                config_train['train_prefetch_factor'] = None
            else:
                train_dataset = EEGData(h5_path, train_folds)
            #train_dataset = EEGData(h5_path, train_folds, train=True)
            ...
            if config_train['val_num_workers'] == 0:
                validation_dataset = CachedEEGData(h5_path, [validation_fold], train=False)
                config_train['val_prefetch_factor'] = None
            else:
            ...
            trained_EEGNet, train_stats, best_model_index, train_losses, val_losses, pred_labels, true_labels = train(
                model, train_dataset, validation_dataset, config_train, writer, device, verbosity=verbosity)

            trained_EEGNet = trained_EEGNet.to('cpu')
            ...
            if train_kf:
                try:
                    ...
                    import time
                    t0 = time.time()
                    RLUD_idx = [1, 0, 2, 3] # indices of output corresponding to right, left, up, down.
                    trained_KF = train_kalmanfilter.train_kf_from_ol(trained_EEGNet, [train_dataset], RLUD_idx)
                    trained_KF.save(model_dir + str(validation_fold) + '_kf.npz')
                    print('finished training KF', time.time()-t0)
                except Exception as e:
                    print('KF training failed!', e)
